...
import uz.sonic.hr.repo.TicketMessageRepository;
import uz.sonic.hr.repo.TicketRepository;
...
public class TicketService {
    ...
    public TicketDetailDto getTicket(Long ticketId, Employee viewer) {
        ...
        if (!ticket.getCreator().getId().equals(viewer.getId()) && !viewer.isAdmin()) {
    ...
    @Transactional
    public TicketMessageDto respondToTicket(Long ticketId, String messageText, Employee admin) {
        if (!admin.isAdmin()) {
    ...
    @Transactional
    public TicketDto updateStatus(Long ticketId, TicketStatus newStatus, Employee admin) {
        if (!admin.isAdmin()) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Admin only");
        }
    ...
    @Transactional
    public TicketDto updatePriority(Long ticketId, TicketPriority priority, Employee admin) {
        if (!admin.isAdmin()) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Admin only");
        }
