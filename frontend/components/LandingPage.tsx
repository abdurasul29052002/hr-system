...
import { LOCALES, LOCALE_PATH, type LandingCopy, type Locale } from '@/lib/landing-copy';
...
/**
 * The public landing page. Copy arrives as a prop from the per-locale server route (a Server Component
 * cannot hand a `t` function to a Client Component, but a plain object serialises fine), so the language
 * is decided by the URL and is present in the server-rendered HTML for crawlers.
 */
export default function LandingPage({ locale, copy }: { locale: Locale; copy: LandingCopy }) {
  ...
  // The root layout renders a single <html lang>, so correct it for the localized routes. Google reads the
  // visible copy and hreflang rather than this attribute, but assistive tech relies on it.
  useEffect(() => {
    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 sm:px-6">
          <Link href={LOCALE_PATH[locale]} className="flex items-center gap-2">
            ...
            <span className="text-base font-bold tracking-tight">{copy.appName}</span>
          </Link>

          <div className="flex-1" />

          {/* Language switcher — real links, so each language is its own crawlable URL. */}
            ...
            {LOCALES.map((l) => (
              <Link
                ...
                className={`rounded-md px-2 py-1 text-xs font-semibold uppercase transition-colors ${
                  l === locale ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {l}
              </Link>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
          <div className="animate-fade">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              >
                {loggedIn ? c.nav.dashboard : c.hero.ctaPrimary}
              </Link>
              {!loggedIn && (
            ...
            <p className="mt-4 text-sm text-slate-400">{c.hero.note}</p>
          </div>

          {/* Product mock: a mini kanban board */}
          <div className="animate-pop lg:justify-self-end">
            <KanbanMock board={copy.board} />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{c.featuresTitle}</h2>
          <p className="mt-3 text-lg text-slate-600">{c.featuresSubtitle}</p>
        </div>
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((key) => (
            <div key={key} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-brand-200 hover:shadow-lg hover:shadow-brand-100/60">
              ...
              <h3 className="mt-4 text-lg font-semibold text-slate-900">{c.features[key].title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{c.features[key].desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-slate-200 bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
          <h2 className="text-center text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{c.howTitle}</h2>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {steps.map((key, i) => (
              <div key={key} className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <h3 className="mt-4 text-lg font-semibold text-slate-900">{c.steps[key].title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">{c.steps[key].desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-20">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-600 to-brand-800 px-6 py-14 text-center shadow-xl shadow-brand-600/20 sm:px-12">
          ...
          <h2 className="relative text-3xl font-bold tracking-tight text-white sm:text-4xl">{c.ctaTitle}</h2>
          <p className="relative mx-auto mt-3 max-w-xl text-lg text-brand-100">{c.ctaSubtitle}</p>
          <Link
            href={loggedIn ? '/tasks' : '/register'}
          ...
          >
            {loggedIn ? c.nav.dashboard : c.ctaButton}
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-200">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 py-8 sm:flex-row sm:px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700">{copy.appName}</span>
            <span className="hidden text-sm text-slate-400 sm:inline">· {c.footerTagline}</span>
          </div>
          <div className="flex items-center gap-5 text-sm text-slate-500">
            <Link href="/login" className="hover:text-slate-900">{c.nav.login}</Link>
            <Link href="/register" className="hover:text-slate-900">{c.nav.getStarted}</Link>
          </div>
        </div>
      </footer>
    </div>
...
function KanbanMock({ board }: { board: LandingCopy['board'] }) {
  const columns = [
    { label: board.open, dot: 'bg-blue-500', cards: [{ p: 'bg-red-400', w: 'w-3/4' }, { p: 'bg-amber-400', w: 'w-1/2' }] },
    { label: board.inProgress, dot: 'bg-amber-500', cards: [{ p: 'bg-amber-400', w: 'w-2/3' }] },
    { label: board.done, dot: 'bg-emerald-500', cards: [{ p: 'bg-slate-300', w: 'w-3/5' }, { p: 'bg-slate-300', w: 'w-1/2' }] },
  ];
  return (
    <div className="w-full max-w-md rotate-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-300/50 ring-1 ring-slate-900/5">
      <div className="mb-3 flex items-center gap-1.5 px-1">
        ...
        <span className="ml-2 text-xs font-medium text-slate-400">{board.title}</span>
      </div>
              ...
              {col.cards.map((card, i) => (
                <div key={i} className="rounded-md border border-slate-200 bg-white p-2 shadow-sm">
                  <div className="mb-1.5 flex items-center gap-1">
                    <span className={`h-1.5 w-1.5 rounded-full ${card.p}`} />
                    <span className={`h-1.5 ${card.w} rounded-full bg-slate-200`} />
                  </div>
